...
| 48 | The month dropdown lists months **newest first**; v1 lists them oldest first | `wordle-teams-l23h` (Phase 7 UI/UX polish) | The owner's call, and a straightforward divergence rather than a fix to anything broken: v1's `getMonthsFromScoreDate` (`src/lib/utils.ts`) walks FORWARD from the starting month and pushes the current month on last, so v1's dropdown ends with the month the reader is almost always after. v2 returns the same set reversed. **Worth more than the three visible rows suggest, which is why it was done before the list grew.** That three-month window was a free-tier affordance and was always temporary — the pro expansion back to the team's earliest score was deferred to the pro gate rather than done here, and `wordle-teams-kusd` has since shipped it, so a Pro subscriber on a migrated v1 team now reads dozens of rows in this dropdown. v1 ALREADY has that expansion and wraps its dropdown in a `ScrollArea` whose height is computed from the option count (`src/components/action-buttons/month-dropdown/utils.ts`), which is the evidence that the list really does get long; ascending order in that shape puts the current month off the bottom of a scroll. Ordering it then meant the expansion inherited the right order instead of rediscovering the problem later, which is what happened: `monthWindowFor` returns newest-first and nothing had to be re-decided. **Scope was ordering only** — not the pro expansion, not a ScrollArea, not the label format, not the size of the window. The direction is now pinned in `v2/convex/lib/monthWindow.test.ts`, whose `monthWindowFor` cases assert `currentMonth` is element 0 and that the window is strictly descending (`month-picker.test.ts` held those properties when the rule lived in the component; `wordle-teams-kusd` deleted it along with `monthOptions`). Strictness matters at length: `PuzzleMonth` is `YYYY-MM`, so lexical comparison is chronological comparison, but a non-increasing list would also satisfy a sort check while giving the radio group two items with one value |
...
| 50 | The Team Boards **day picker reaches every month the dropdown offers**, and opens on the day being viewed | `wordle-teams-5vv3` (Phase 7 UI/UX polish) | **Two owner complaints about one control, and the first is a bug v1 shares.** (1) The calendar always opened on the CLOCK'S month: react-day-picker resolves its initial month as `month \|\| defaultMonth \|\| today` and never consults `selected` (react-day-picker's own `helpers/getInitialMonth.js`), and `date-picker.tsx` passed neither — so a viewer looking at a July day had to page back two months to reach the day already on screen behind the popover. Filed as `wordle-teams-p5mw` at the Task 10 review and closed by this. v1 renders the same component with the same props, so it has the identical defect; it is simply reached less often there. (2) The picker was clamped to the LOADED month with `minDay`/`maxDay`, on the sound reasoning that `getTeamMonth` holds one month — but the right conclusion was that reaching another month is a NAVIGATION, not a day with no data. It now bounds on the month window's own output (`monthWindowFor`, `v2/convex/lib/monthWindow.ts`), passed down from `routes/app.tsx` as the SAME array the MonthPicker is driven by, and a pick outside the loaded month moves `?month=` through an `onMonthChange` callback. **Bounded to the dropdown's window rather than unbounded like board entry's, deliberately:** v2 had no pro month gate when this was written — the dropdown returned three months to everyone — so an unbounded picker would have handed every player unlimited history and the pro expansion would later have had to take it away. `wordle-teams-kusd` shipped that gate, and because one array drives both controls they widened together on the day it landed rather than needing a second change here; they still cannot disagree about what exists, and `team-boards.hook.test.ts` parses `routes/app.tsx` and asserts the two `months` attributes are the same expression. **`maxDay` was DELETED rather than widened**, restoring `date-picker.tsx`'s own no-future-days default, which its doc comment already states `maxDay` can never widen. The Previous/Next DAY arrows were left stopping at the month boundary here and crossed in `wordle-teams-5nmo` immediately after — see row 51 |
...
| 62 | The scores table marks the caller's own row with a **background tint across the full row**; v1 has no equivalent marker | `wordle-teams-5jcn` (T5, revised T20) | `<TableRow>` carries a plain `bg-muted/50` when the row is the caller's — deliberately the same class `ui/table.tsx`'s own `hover:bg-muted/50` already uses; the owner accepted that collision from a screenshot rather than have a ring on just the pinned cells, which "looked terrible" (`wordle-teams-5jcn.20`). The scrolling day `<td>`s carry no background of their own, so the row's tint shows straight through them for free. The two pinned cells cannot reuse that same class: they already carry an opaque `bg-background` (so the day columns can't show through a sticky cell), and tailwind-merge collapses two `bg-*` utilities in one class string down to the last one — a naive `cn(pinned, 'bg-muted/50')` silently deletes `bg-background`, leaving the pinned cells translucent on exactly the row a reader is most likely to be looking at (the `wt-ksh.3.16` bug, recurring). `bg-pinned-self` (`styles.css`) is a single opaque `color-mix` composite of the same `--muted`/`--background` tokens, handed to tailwind-merge as one class so it reliably replaces `bg-background` instead of racing it, and reads as the tint sitting over the pinned cell's base without two `bg-*` utilities ever sitting in one string to conflict |
...
- **The month picker offered a fixed three-month window to everyone — CLOSED by
  `wordle-teams-kusd`.** v1 widens it for pro accounts; Phase 2 deliberately
  deferred the pro gate, so v2 showed a pro user *less* history than prod. It
  was the safe direction — nobody ever saw more than v1 would allow — but it was
  a real divergence rather than one that only looked like one, which is why it
  is the entry in this list that got fixed instead of accepted. The rule now
  lives in `v2/convex/lib/monthWindow.ts`; `routes/app.tsx` drives both the
  month dropdown and the Team Boards day picker from it, and `getTeamMonthFor`
  (`v2/convex/scores.ts`) and `lastMonthWinnerFor` (`v2/convex/winners.ts`)
  refuse a below-floor month with `MONTH_OUT_OF_WINDOW`. Kept here rather than
  deleted so the parity audit reads "found and closed" rather than re-deriving
  it.
