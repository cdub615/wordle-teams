...
and rows 25-30 by Task 9's, and the header above still says eighteen.** That is
deliberate, not an oversight: **Task 16** owns the parity audit and will add
rows of its own, so it reconciles the total in one place rather than the count
drifting a task at a time on the way there. Task 16 must fold all twelve into
the header count. Nothing above them was renumbered, and no row was renumbered
when 22-24 or 25-30 were added.
