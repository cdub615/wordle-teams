...
AS $$
  ...
  WHERE p.time_zone IS NOT NULL
    ...
    AND (
      EXTRACT(DOW FROM CURRENT_DATE) NOT IN (0, 6)
      OR EXISTS (SELECT 1 FROM teams WHERE p.id = ANY(player_ids) AND play_weekends = true)
    )
    AND EXISTS (
      SELECT 1
      FROM daily_scores ds
      WHERE ds.player_id = p.id
        AND ds.date >= CURRENT_DATE - INTERVAL '10 days'
    )
