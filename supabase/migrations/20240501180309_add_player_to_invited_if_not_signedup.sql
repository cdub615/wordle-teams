...
DECLARE
    ...
    invited_email text;
BEGIN
    select p.id, p.first_name, case when pc.membership_status = 'pro' then true else false end pro_member, count(distinct t.id) as team_count
    into pid, first_name, pro_member, team_count
    ...
    IF first_name IS NULL THEN
        ...
        UPDATE public.teams
        SET invited = array_append(invited, invited_email)
        WHERE id = team_id_input;
    ELSEIF NOT pro_member AND team_count >= 2 THEN
