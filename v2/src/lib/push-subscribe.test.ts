...
describe('urlBase64ToUint8Array', () => {
  ...
  test('decodes an already-padded string too', () => {
    // `(4 - 0 % 4) % 4` must be 0 rather than 4. Nothing stops a hand-set
    // VAPID_PUBLIC_KEY carrying its padding, and four spurious '=' make atob
    // throw — a Push switch that fails on one deployment and not another.
    expect(Array.from(urlBase64ToUint8Array('YWJjZA=='))).toEqual([97, 98, 99, 100])
  })
...
describe('unsubscribeFromPush', () => {
  ...
  test('a browser with no push manager at all is a no-op too', async () => {
    ...
    // cannot do push — the caller does that unconditionally, so this must not
    // throw its way out of the handler first.
