...
if (probe.copied.withTimeZone > 0) {
  ...
  console.log('      on its own — eligibility needs a delivery method too.')
  console.log('')
  console.log('      TWO MECHANISMS PRODUCE THIS AND THIS COUNT CANNOT SEPARATE THEM.')
  console.log('      (1) A copy that predates copy-reminder-policy.mjs carried timeZone for')
  console.log('          every player it wrote; the early runs were --scope=mine.')
  console.log('      (2) src/lib/use-local-capture.ts WRITES ONE ON SIGN-IN to any player')
  console.log('          lacking one (Header.tsx mounts it in __root.tsx).')
  console.log('')
  console.log('      SO "CLEAR THEM DELIBERATELY" IS ONLY HALF AVAILABLE: anything cleared is')
  console.log('      rewritten the next time that player signs in. See wordle-teams-k501')
  console.log('      before planning any cleanup.')
}
